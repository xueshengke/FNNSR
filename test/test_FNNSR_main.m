...
weights = 'model/x2_d4_c5_k5_iter_5e4.caffemodel';
...
patch_size = [360, 480]; % cannot be too large
stride = patch_size;
border = scale; 
...
%% process image files by FNNSRNet
...
for i = 1 : length(dic)   
    ...
    image_res_batch_ht = zeros(size(image_bic_batch_ht));
    for t = 1 : size(image_bic_batch_ht, 3)
        image_res_batch_ht(:, :, t) = FNNSRNet(model, weights, ...
            image_bic_batch_ht(:, :, t));
    end
